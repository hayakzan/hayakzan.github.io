...
OK, let's see. Finding median is one thing [binary search ](https://www.youtube.com/watch?v=KXJSjte_OAI) algorithm is very good at. So let's try to implement [a portion of] it.

First off, we need to scan through the arrays. We're going to do this based on the smallest array, in case `nums1` and `nums2` are not of equal size. `nums1` will be used for the rest of the algorithm, but if it's bigger than `nums2`, we will swap them using the help of a `temp` array so that we can keep using `nums1`.
...
```
Now we can set the conditions on our
